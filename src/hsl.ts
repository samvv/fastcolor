...
export type HSL = [h: number, s: number, l: number, a?: number] & { readonly [colorTag]: 'hsl' };

export namespace HSL {

  export function create(h: number, s: number, l: number, a?: number): HSL {
    if (h < 0.0 || h > 360.0) {
      throw new Error(`H component must be between 0 and 360, inclusive.`);
    }
    if (s < 0.0 || s > 1.0) {
      throw new Error(`S component must be between 0 and 1, inclusive.`);
    }
    if (l < 0.0 || l > 1.0) {
      throw new Error(`L component must be between 0 and 1, inclusive.`);
    }
