...
export type RGB = [r: number, g: number, b: number, a?: number] & { readonly [colorTag]: 'rgb' };

export namespace RGB {

  export function create(r: number, g: number, b: number, a?: number): RGB {
    if (r < 0.0 || r > 1.0) {
      throw new Error(`R value must be between 0 and 1, inclusive.`);
    }
    if (g < 0.0 || g > 1.0) {
      throw new Error(`G value must be between 0 and 1, inclusive.`);
    }
    if (b < 0.0 || b > 1.0) {
      throw new Error(`B value must be between 0 and 1, inclusive.`);
    }
